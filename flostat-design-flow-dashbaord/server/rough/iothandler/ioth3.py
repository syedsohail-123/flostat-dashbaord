...
logsTable = dynamodb.Table("AWS_LogsTable_Table")
...
def log_to_dynamodb(data):
    """Insert log entry in AWS_LogsTable_Table."""
    log_id = str(uuid.uuid4())
    timestamp = datetime.datetime.utcnow().isoformat()

    data["uuid"]= log_id
    print("Data entry: "+str(data))
    print("Created log entry: "+log_id)

    logsTable.put_item(Item=data)
    return log_id
...
def handle_schedule_update_ack(data):
    ...
    if attributes.get("pump_ack") and attributes.get("valve_ack"):
        ...
        log_to_dynamodb(res2.get("Attributes", {}))
...
def handle_schedule_delete_ack(data):
    ...
    if attributes.get("pump_ack") and attributes.get("valve_ack"):
        res2 = table.delete_item(Key={"org_id": org_id, "schedule_id": schedule_id})
        log_to_dynamodb(res2.get("Attributes", {}))
