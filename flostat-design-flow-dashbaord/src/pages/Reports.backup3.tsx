...
export default function Reports() {
  const { authToken, currentOrganization, organizations, setCurrentOrganization } = useAuth(); // Get auth token, current organization, and organizations from context
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="p-4 rounded-md border bg-warning/10 border-warning/30 text-xs text-soft">
        <div className="font-medium text-center text-warning">Authentication Status</div>
        <div className="mt-2 grid gap-1 sm:grid-cols-2">
          <div>Auth Token: {authToken ? "Present" : "Missing"}</div>
          <div>Current Organization: {currentOrganization ? currentOrganization.name : "None"}</div>
      ...
      <div className="flex items-end justify-between gap-3">
        <div className="flex items-end gap-3">
          ...
          <Button className="h-9 px-3 bg-[hsl(var(--aqua))] hover:bg-[hsl(var(--aqua))]/90 text-white shadow-soft-sm" onClick={handleFetchData}>Fetch Data</Button>
          <Button 
      ...
      {authToken && organizations && organizations.length > 0 && !currentOrganization && (
        <div className="p-4 rounded-md border bg-warning/10 border-warning/30">
          <div className="font-medium text-center text-warning mb-2">Select an Organization</div>
          <p className="text-center text-sm mb-3">You need to select an organization to view real-time reports.</p>
          <div className="flex justify-center gap-2">
            <Select onValueChange={(value) => {
              const org = organizations.find(o => o.org_id === value);
              if (org) {
